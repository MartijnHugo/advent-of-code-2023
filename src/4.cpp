...
int main() {
    ...
    {
        Card one_card(cardSummary);
        sum_score += one_card.score();
    ...
    do {
        copies = {};
        for (int i = checked_cards; i < cards.size(); i++) {
            int nrOfWinningNumbers = cards[i].nrOfWinningNumbers();
            int cardId = cards[i].getCardId();
            for (int j = cardId; j < cardId + nrOfWinningNumbers; j++) {
                copies.push_back(Card(cards[j]));
            }
            checked_cards++;
        }
        cards.insert(cards.end(), copies.begin(), copies.end());
    } while (copies.size() > 0);

    std::cout << "Answer part 1: " << sum_score << std::endl;
    std::cout << "Answer part 2: " << cards.size() << std::endl;
