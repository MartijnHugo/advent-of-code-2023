...
const std::string INPUT_FILE = "etc/8.txt";

class Node
{
public:
    Node(std::string code, Node *left_node = nullptr, Node *right_node = nullptr)
     : code_(code), left_node_(left_node), right_node_(right_node) {};

    void setLeftNode(Node *left_node) { left_node_ = left_node; }
    void setRightNode(Node *right_node) { right_node_ = right_node; }

    Node *getLeftNode() { return left_node_; }
    Node *getRightNode() { return right_node_; }
...
int main()
{
    ...
    for (std::string line; std::getline(file, line); ) {
        if (line == "") continue;
            ...
            std::vector<char> instructions_raw(line.begin(), line.end());
            instructions.insert(instructions.end(), instructions_raw.begin(), instructions_raw.end());
            continue;
        }

        std::string node_code = line.substr(0, 3);
        std::string left_code = line.substr(7, 3);
        std::string right_code = line.substr(12, 3);
        if (node_addresses.find(node_code) == node_addresses.end()) {
            node_addresses[node_code] = new Node(node_code);
        }
        if (node_addresses.find(left_code) == node_addresses.end()) {
            node_addresses[left_code] = new Node(left_code);
        }
        if (node_addresses.find(right_code) == node_addresses.end()) {
            node_addresses[right_code] = new Node(right_code);
        }
        Node *this_node_address = node_addresses[node_code];
        Node *left_node_address = node_addresses[left_code];
        Node *right_node_address = node_addresses[right_code];

        (*this_node_address).setLeftNode(left_node_address);
        (*this_node_address).setRightNode(right_node_address);
    }

    Node *current_node = node_addresses["AAA"];
    int counter = 0;
    while ((*current_node).getCode() != "ZZZ") {
        char current_instruction = instructions[counter % instructions.size()];
    ...
    for (auto const& [code, address] : node_addresses) {
        if (code.at(2) == 'A') {
            input_nodes.push_back(address);
        }
    }

    std::vector<unsigned long long int> steps_needed;
    for (Node *address : input_nodes) {
        current_node = address;
        unsigned long long int ghost_counter = 0;
        while ((*current_node).getCode().at(2) != 'Z') {
            char current_instruction = instructions[ghost_counter % instructions.size()];
            if (current_instruction == 'L') {
                current_node = (*current_node).getLeftNode();
            } else {
                current_node = (*current_node).getRightNode();
        ...
        steps_needed.push_back(ghost_counter);
    }

    unsigned long long int result = steps_needed[0];
    for (int i = 1; i < steps_needed.size(); i++) {
        result = std::lcm(result, steps_needed[i]);
    }
