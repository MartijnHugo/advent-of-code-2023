...
const std::string INPUT_FILE = "etc/9.txt";
...
int main()
{
    ...
    for (std::string line; std::getline(file, line); ) {
        ...
        for (std::string valueStr; std::getline(historyStream, valueStr, ' '); ) {
            sequence.push_back(std::stoi(valueStr));
        }

        std::vector<int> input_sequence = sequence;
        std::vector<std::vector<int>> sequences = {input_sequence};
        while (std::any_of(input_sequence.begin(), input_sequence.end(), [](int value) { return value != 0; })) {
            std::vector<int> output_sequence = {};
            for (int i = 1; i < input_sequence.size(); i++) {
                int difference = input_sequence[i] - input_sequence[i-1];
                output_sequence.push_back(difference);
            }
            input_sequence = output_sequence;
            sequences.push_back(output_sequence);
        }

        int future_value, history_value;
        for (int i = sequences.size() - 1; i >= 0; i--) {
            std::vector<int> seq = sequences[i];
            if (i == sequences.size() - 1) {
                future_value = 0;
                history_value = 0;
            } else {
                future_value += seq[seq.size() - 1];
                history_value = seq[0] - history_value;
            }

            if (i == 0) {
                future_sum += future_value;
                history_sum += history_value;
            }
        }
    }

    std::cout << "Answer part 1: " << future_sum << std::endl;
    std::cout << "Answer part 2: " << history_sum << std::endl;
